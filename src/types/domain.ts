...
export interface RecyclerProfile { recyclerId: string; facility: string; verificationStatus: VerificationStatus; acceptedMaterials: string[]; processingCapabilities: string[]; capacity: number; location: string }
...
export interface Transaction { transactionId: string; lotId: string; collectorId: string; recyclerId: string; offerId: string; materialSummary: string; amount: number; status: LotStatus; createdAt: string; updatedAt: string }
...
export interface AuditLog { eventId: string; lotId: string; transactionId?: string; actorId: string; actorRole: UserRole; eventType: AuditEventType; timestamp: string; metadata?: Record<string, unknown> }
