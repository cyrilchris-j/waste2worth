    ...
    <div className="min-h-screen bg-gray-50 max-w-2xl mx-auto">
      ...
      <header className="bg-gray-900 text-white px-4 pt-8 pb-5">
        ...
        <div className="flex gap-2 mt-4 border-b border-gray-800 pb-2">
          <span className="text-xs font-semibold text-white border-b-2 border-brand-500 pb-1">
            Verification Queue
          </span>
          <a href="/admin/intelligence" className="text-xs text-gray-400 hover:text-white pb-1">
            Matching & Pricing Intelligence
          </a>
          <a href="/trace/LOT-2026-001" className="text-xs text-gray-400 hover:text-white pb-1">
            Public QR Trace
          </a>
        </div>
        ...
        {profiles.map((profile) => (
          <div key={profile.recyclerId} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {(NEXT_STATUS[String(profile.verificationStatus)] ?? []).map((nextStatus: VerificationStatus) => (
                <Button
                  key={nextStatus}
                  size="sm"
