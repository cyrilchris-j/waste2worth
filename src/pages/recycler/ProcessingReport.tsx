        ...
        <div className="page-container">
          ...
          {existingReport.recoveredMaterials?.length > 0 && (
            <div className="section-card">
              ...
              {existingReport.recoveredMaterials.map((rm: any, i: number) => (
                <div key={i} className="info-row">
                  <span className="info-label">{rm.material || rm.name || 'Material'}</span>
                  <span className="info-value">{rm.quantityKg ?? rm.weightKg ?? 0} kg</span>
                </div>
              ))}
            </div>
          )}
