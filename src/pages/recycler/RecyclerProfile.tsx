...
export default function RecyclerProfile() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateRecyclerProfile(recyclerProfile.userId, {
          ...
          <>
            <Card>
              ...
              <div className="flex flex-wrap gap-2">
                {recyclerProfile.acceptedCategories.map((cat) => (
                  <span key={cat} className="chip chip-green">{cat}</span>
                ))}
                {recyclerProfile.acceptedCategories.length === 0 && <p className="text-sm text-gray-400">None declared</p>}
              </div>
            </Card>
            ...
            <Card>
              ...
              <div className="flex flex-wrap gap-2">
                {recyclerProfile.processingCapabilities.map((cap) => (
                  <span key={cap} className="chip">{cap}</span>
                ))}
                {recyclerProfile.processingCapabilities.length === 0 && <p className="text-sm text-gray-400">None declared</p>}
              </div>
            </Card>
          </>
        )}
