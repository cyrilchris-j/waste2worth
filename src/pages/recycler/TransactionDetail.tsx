    ...
    <RecyclerLayout title={`TXN ${txn.transactionId}`} backPath="/recycler/transactions">
      <div className="page-container">
        ...
        <div className="section-card">
          ...
          {[
            { label: 'E-Waste Value',     value: formatCurrency(txn.agreedPrice) },
            { label: 'Delivery / Pickup', value: formatCurrency(txn.deliveryCost) },
            { label: 'Platform Fee',      value: formatCurrency(txn.platformFee) },
          ].map(({ label, value }) => (
            <div key={label} className="info-row">
              <span className="info-label">{label}</span>
              <span className="info-value">{value}</span>
          ...
          <div className="border-t border-gray-100 pt-2 flex justify-between items-center">
            ...
            <span className="text-lg font-bold text-brand-700">{formatCurrency(txn.totalAmount)}</span>
          </div>
